...
// New creates a new RotateLogs object. A core filename pattern
...
func (rl *RotateLogs) getWriterNoLock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	// ./foo/bar/baz/hello.core must make sure ./foo/bar/baz is existed
...
// Rotate forcefully rotates the core files. If the generated file name
// clash because file already exists, a numeric suffix of the form
// ".1", ".2", ".3" and so forth are appended to the end of the core file
//
// Thie method can be used in conjunction with a signal handler so to
// emulate servers that generate new core files when they receive a
// SIGHUP
